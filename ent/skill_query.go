...
func (sq *SkillQuery) QueryUsers() *MemberQuery {
	query := (&MemberClient{config: sq.config}).Query()
...
func (sq *SkillQuery) QueryTeams() *TeamQuery {
...
func (sq *SkillQuery) WithUsers(opts ...func(*MemberQuery)) *SkillQuery {
	query := (&MemberClient{config: sq.config}).Query()
...
func (sq *SkillQuery) WithTeams(opts ...func(*TeamQuery)) *SkillQuery {
	query := (&TeamClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := sq.withUsers; query != nil {
		if err := sq.loadUsers(ctx, query, nodes,
			func(n *Skill) { n.Edges.Users = []*Member{} },
			func(n *Skill, e *Member) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *SkillQuery) loadUsers(ctx context.Context, query *MemberQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Member)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(skill.UsersTable)
		s.Join(joinT).On(s.C(member.FieldID), joinT.C(skill.UsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(skill.UsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(skill.UsersPrimaryKey[0]))
...
func (sq *SkillQuery) loadTeams(ctx context.Context, query *TeamQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Team)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Skill)
	nids := make(map[int]map[*Skill]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(team.FieldID), joinT.C(skill.TeamsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(skill.TeamsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(skill.TeamsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
